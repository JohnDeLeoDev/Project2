...
class Model {
    ...
    translateToParent = translate(0, 0, 0) // The translation of the model to the parent object
    ...
    groupVector = vec3(0, 1, 0) // Points forward from the center of the parent object
    ...
    setTranslation(translation) {
        ...
        for (let key in this.embeddedObjects) {
            let parentTranslation = this.translation
            let parentRotation = this.rotation
            let parentScale = this.scale

            let childrelativeTranslation =
                this.embeddedObjects[key].translateToParent

            let childTranslation = mat4()
            childTranslation = mult(parentRotation, childrelativeTranslation)

            childTranslation = mult(parentTranslation, childTranslation)

            this.embeddedObjects[key].setTranslation(childTranslation)
        }
    }
    ...
    getPosition() {
        let x = this.getTranslation()[0][3]
        let y = this.getTranslation()[1][3]
        let z = this.getTranslation()[2][3]
        return [x, y, z]
    }

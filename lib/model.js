 ...
 */
class Model {
    name = null // The name of the model
    ...
    texImage = null // Image object for texture

    buffers = {} // Buffers for the model

    diffuseColors = null // The diffuse colors of the model
    specularColors = null // The specular colors of the model
    ambientColors = null // The ambient colors of the model

    translation = translate(0, 0, 0) // The translation of the model
    rotation = rotateY(0) // The rotation of the model
    scale = scalem(1, 1, 1) // The scale of the model
    shadowMatrix = mat4() // The shadow transformation matrix of the model
    shadowTranslation = translate(0, 0, 0) // The translation of the shadow

    embeddedObjects = {} // The embedded objects in the model

    transToParent = mat4()
    rotToParent = mat4() // The rotation matrix relative to the parent object (if any)
    scaleToParent = mat4() // The scale matrix relative to the parent object (if any)
    parentTranslation = mat4() // The translation matrix of the parent object (if any)
    parentRotation = mat4() // The rotation matrix of the parent object (if any)
    parentScale = mat4() // The scale matrix of the parent object (if any)

    constructor(name, objPath, mtlPath) {
    ...
    // checks if the model is loaded and ready to be used
    load() {
    ...
    parseMtlFile(mtlFile) {
        ...
        for (let currLine = 0; currLine < mtlLines.length; currLine++) {
            ...
            if (line.startsWith('newmtl')) {
                ...
                this.texImage.src = this.imagePath
            }
        }
    ...
    processFace(line, vertices, normals, uvs, currMaterial) {
        ...
        if (types === 0) {
            ...
            indices.forEach((value) => {
                ...
                // Vertex coordinates
                faceVerts.push(
                    this.getVertex(value, 0, firstSlashIndex, vertices)
                )

                // Texture coordinates
                faceTexs.push(
                    this.getVertex(value, firstSlashIndex + 1, -1, uvs)
                )
            })
        } else if (types === 2) {
    ...
    // get and flatten normals
    getNormals() {
        let normals = []
        this.faces.forEach((face) => {
            face.faceNormals.forEach((norm) => {
                normals.push(norm)
            })
        })
    ...
    // get and flatten texture coordinates
    getTexCoords() {
        let texs = []
        this.faces.forEach((face) => {
            face.faceTexCoords.forEach((tex) => {
                texs.push(tex)
            })
        })
        return flatten(texs)
    ...
    getDiffuseColors() {
        ...
        for (let face of this.faces) {
            for (let vertex of face.faceVertices) {
                colors.push(this.diffuseMap.get(face.material))
            }
        }
    ...
    getSpecularColors() {
        ...
        for (let face of this.faces) {
            for (let vertex of face.faceVertices) {
                colors.push(this.specularMap.get(face.material))
            }
        }
        return flatten(colors)
    }
    ...
    // Base all movements on the parent object, if one exists
    setTranslation(translation) {
        this.translation = translation

        for (let obj in this.embeddedObjects) {
            this.embeddedObjects[obj].setParentTranslation(translation)
        }
    }

    setParentTranslation(translation) {
        this.parentTranslation = translation
    }
    ...
    setRotation(rotation) {
        ...
        for (let obj in this.embeddedObjects) {
            this.embeddedObjects[obj].setParentRotation(rotation)
        }
    }

    // takes a scale matrix and applies it to the object
    setScale(scale) {
        this.scale = scale

        for (let obj in this.embeddedObjects) {
            this.embeddedObjects[obj].setParentScale(scale)
        }
    }
    ...
    getTranslation() {
        if (this.type == 'parent') {
            ...
            return mult(
                this.parentTranslation,
                mult(this.translation, this.transToParent)
            )
        }
    }
    ...
    getRotation() {
        if (this.type == 'parent') {
            ...
            return mult(
                this.parentRotation,
                mult(this.rotation, this.rotToParent)
            )
        }
    }
    ...
    getRotationAngle() {
        ...
        let xDegrees = Math.asin(rotation[2][1])
        let yDegrees = Math.asin(rotation[0][2])
        let zDegrees = Math.asin(rotation[1][0])

        // convert to degrees
        xDegrees = (180 * xDegrees) / Math.PI
        yDegrees = (180 * yDegrees) / Math.PI
        zDegrees = (180 * zDegrees) / Math.PI

        return [xDegrees, yDegrees, zDegrees]
    }
}

...
class Model {
    ...
    materialColors = null;      // The diffuse colors of the model

    translation = mat4();         // The translation of the model
    rotation = mat4();            // The rotation of the model
    scale = mat4();               // The scale of the model
    ...
    constructor(objPath, mtlPath) {
        ...
        this.name = objPath.substring(objPath.lastIndexOf('/') + 1, objPath.lastIndexOf('.'));
    ...
    // get and flatten diffuse colors
    getMaterialColors() {
        let colors = [];
        for (let i = 0; i < this.faces.length; i++) {
            for (let j = 0; j < this.faces[i].faceVertices.length; j++) {
                colors.push(this.diffuseMap.get(this.faces[i].material));
            }
        }
        return colors;
    }

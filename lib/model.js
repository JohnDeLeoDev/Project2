...
class Model {
    ...
    buffers = null // Buffer for the model
    ...
    currentTransToParent = mat4() // The original translation matrix
    transToParent = mat4() // The transformation matrix relative to the parent object (if any
    ...
    setTranslation(translation) {
        ...
        for (let obj in this.embeddedObjects) {
            this.embeddedObjects[obj].setTranslation(translation)
        }
    }
    ...
    setRotation(rotation) {
        ...
        // take current transToParent Matrix and multiply it by the new rotation matrix so that the rotation is relative to the parent object
        for (let obj in this.embeddedObjects) {
            this.embeddedObjects[obj].setRotation(rotation)
            this.embeddedObjects[obj].rotateTransToParent(rotation)
        }
    }
    ...
    setScale(scale) {
        ...
        for (let obj in this.embeddedObjects) {
            this.embeddedObjects[obj].setScale(scale)
        }
    }

    rotateTransToParent(rotation) {
        this.currentTransToParent = mult(rotation, this.transToParent)
    }

    getTranslation() {
        return mult(this.translation, this.currentTransToParent)
    }

    getRotation() {
        return mult(this.rotation, this.rotToParent)
    }
